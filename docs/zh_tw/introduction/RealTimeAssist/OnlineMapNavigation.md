...
```python
read(max_age: float = 1.0) -> tuple[float, float, float] | None
```
...
| 參數 | 類型 | 預設值 | 說明 |
| --- | --- | --- | --- |
| `max_age` | `float` | `1.0` | 允許返回的座標最大快取時間（秒）。最近一次座標的擷取時間距今不超過該值時返回座標，否則返回 `None`。 |

返回值為 `(x, y, z)`，三個值皆為 `float`。若尚未啟動、尚未收到有效座標、座標過期、傳送或切換實例時資料中斷，或目前封包中沒有可識別座標，則返回 `None`。
...
正常移動時，座標會隨角色位置持續更新。傳送、切換地圖或實例、切換角色、網路重連、遊戲移動時間戳重置或遊戲切換到新的網路流時，可能短暫返回 `None`。介面會嘗試重新識別有效移動流，呼叫方不應因一次 `None` 就立即銷毀並重建實例。
